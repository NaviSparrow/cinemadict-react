...
function CommentsList({commentsList}:CommentsListProps) {
  return (
    <>
      <h3 className="film-details__comments-title">Comments <span className="film-details__comments-count">{commentsList.length}</span></h3>
      <ul className="film-details__comments-list">
        {commentsList.map((comment:CommentType) => <Comment key={comment.id} commentData={comment} />)}
      </ul>
    </>
  );
}
